...
class HomeNewVisitorTest(StaticLiveServerTestCase):
    ...
    def test_home_title(self):
        self.browser.get(self.get_full_url(self.browser.title))
        self.assertIn("taskBuster", self.browser.title)

    def test_h1_css(self):
        self.browser.get(self.get_full_url("home"))
        h1 = self.browser.find_element_by_tag_name("h1")
        self.assertEqual(h1.value_of_css_property("color"), u"rgb(200, 50, 255)")
    ...
    def test_localization(self):
        ...
        for lang in ['en', 'de']:
            activate(lang)
            self.browser.get(self.get_full_url("home"))
            local_date = self.browser.find_element_by_id('non-local-date')
...
# run unit and functional tests using coverage
